...
public:
    ...
    BinTreeNode* find(BinTreeNode* t, T& item);
    void insert(BinTreeNode* t, T item);
...
/*
BinTreeNode* BinTree::copyTree(BinTreeNode *t) {
    if(t == nullptr){
        return nullptr;
    }
    ...
    BinTreeNode *newNode = new BinTreeNode(t->data, newLptr, newRptr);
...
BinTreeNode* BinTree::father(BinTreeNode *t, BinTreeNode *p) {
    ...
    if((q = father(t->left, p)) != nullptr)
        return t;
    else
        return father(t->right, p);
}

BinTreeNode* BinTree::find(BinTreeNode *t, T &item) {
    BinTreeNode* p;
    BinTreeNode* q;
    ...
    if(t->data == item) // 判断数据域
        return t;

    if(((p = find(t->left, item)) != nullptr))
        return p;
    else
        return q = find(t->right, item);
}

void BinTree::insert(BinTreeNode *t, T item) {
    if(t == nullptr)
        return;

    BinTreeNode* p = new BinTreeNode(item, t->left);
    t->left = p;
